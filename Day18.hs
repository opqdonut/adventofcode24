...
bfs start end chart = go S.empty [(start,0,[start])]
  where go visited ((p,d,path):qs)
          | p==end = Just (d,path)
          | S.member p visited = go visited qs
          | otherwise = go (S.insert p visited) (qs ++ [(n,d+1,n:path) | n <- neighbours p
                                                                       , S.member n chart
...
part2 inp = go allPositions initialPath inp
  where mkChart cs = S.difference allPositions $ S.fromList cs
        ...
        go chart path (i:inp)
          | notElem i path = traceShow ("ignore",i) $ go chart' path inp
          | otherwise = case traceShow ("delete",i,S.size chart') $ bfs (0,0) (70,70) chart'
                        of Just (_,path') -> traceShow path $ go chart' path' inp
                           Nothing -> i
